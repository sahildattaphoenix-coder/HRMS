...
export class ProjectService {
  ...
  getProject(id: string | number): Observable<Project> {
    return this.apiService
      .get<Project>(`${this.ENDPOINT}/${id}`)
      .pipe(map((data) => data as unknown as Project));
  }
  ...
  getMyProjects(userId: string | number): Observable<Project[]> {
    return this.apiService.get<Employee>('employees').pipe(
      map((employees) =>
        employees.find((e) => e.userId.toString() === userId.toString())
      ),
      switchMap((employee) => {
        if (!employee) {
          return of([]);
          ...
          map((projects) =>
            projects.filter(
              (p) =>
                p.team &&
                p.team.some(
                  (member) => member.id.toString() === employee.id.toString()
                )
            )
          )
        );
  ...
  addProject(project: Project): Observable<Project> {
    return this.apiService
      .post<Project>(this.ENDPOINT, project)
  ...
  updateProject(
    ...
    return this.apiService
      .put<Project>(this.ENDPOINT, id, project)
      .pipe(tap(() => this._refresh$.next()));
  }
