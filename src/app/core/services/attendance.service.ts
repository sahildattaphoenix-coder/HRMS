...
export class AttendanceService {
  ...
  getTodayAttendance(employeeId: string): Observable<Attendance | null> {
    ...
    return this.apiService
      ...
      .pipe(
        map(
          (list) =>
            list.find((a) => a.employeeId == employeeId && a.date == today) ||
            null
        )
      );
  }
  ...
  clockOut(
    ...
    return this.apiService.patch<Attendance>('attendance', id, {
      checkOut,
      hours,
    });
  }
  ...
  getAttendanceByEmployee(employeeId: string): Observable<Attendance[]> {
    return this.apiService
      .get<Attendance>('attendance')
      .pipe(map((list) => list.filter((a) => a.employeeId == employeeId)));
  }
  ...
  getActiveAttendance(employeeId: string): Observable<Attendance | null> {
    return this.apiService
      ...
      .pipe(
        map(
          (list) =>
            list.find((a) => a.employeeId == employeeId && !a.checkOut) || null
        )
      );
  }
}
