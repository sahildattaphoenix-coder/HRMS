...
export class ActionNotificationService {
  ...
  private checkNewPendingActions() {
    this.apiService.get<Timesheet>('timesheets').subscribe({
        ...
        pending.forEach((t) => {
          if (!this.processedTimesheetIds.has(t.id)) {
            this.toastService.info(
              `${t.status} Timesheet: ${t.employeeName}`,
              3000
            );
      ...
      error: (err) =>
        console.error(
          'ActionNotificationService: Error fetching timesheets',
          err
        ),
    });
    ...
    this.apiService.get<any>('leaveRequests').subscribe({
      next: (data) => {
        const pending = data.filter(
          (l: any) => l.status === 'Pending' || l.status === 'Draft'
        );

        pending.forEach((l: any) => {
          if (!this.processedLeaveIds.has(l.id)) {
            this.toastService.info(
              `${l.status} Leave: ${l.employeeName} (${l.leaveType})`,
              3000
            );
      ...
      error: (err) =>
        console.error(
          'ActionNotificationService: Error fetching leave requests',
          err
        ),
    });
  }
